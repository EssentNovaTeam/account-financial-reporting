...
class CommonReportHeaderWebkit(common_report_header):
    ...
    def _organize_by_account(data):
        """ Extract account_id key from list of account data dictionary and
        map the additional values to the account_id. """
        res = {}
        for x in data:
            res[x.get('account_id')] = {
                'credit': x['credit'],
                'debit': x['debit'],
                'balance': x['balance'],
    ...
    @staticmethod
    def update_account_data(res, update_data):
        """ Combine 2 dictionaries, each with account_id as key"""
        for acc_id, values in update_data.items():
            res_entry = res[acc_id]
            credit = values['credit'] + res_entry['credit']
            debit = values['debit'] + res_entry['debit']
        ...
        if not default_values:
            try:
                self.cursor.execute("""
                    ...
                    GROUP BY account_id;
                """, (tuple(account_ids), tuple(period_ids)))
                result = self._organize_by_account(
    ...
    def _compute_initial_balances(self, account_ids, start_period, fiscalyear):
        ...
        # Construct default values for each account
        res = self._compute_init_balance(account_id=account_ids,
                                         default_values=True)
        ...
        if pnl_periods_ids and not opening_period_selected:
            res = self.update_account_data(res, self._compute_init_balance(
