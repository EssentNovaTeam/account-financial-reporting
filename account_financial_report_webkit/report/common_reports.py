...
class CommonReportHeaderWebkit(common_report_header):
    ...
    def update_account_data(res, update_data):
        """ Merge update_data into res """
        for acc_id, values in update_data.items():
            entry = res.get(acc_id)
            if not entry:
                # The entry does not exist yet, just insert values
                res[acc_id] = values
                continue
            res[acc_id].update({
                'credit': values['credit'] + entry['credit'],
                'debit': values['debit'] + entry['debit'],
                'balance': values['balance'] + entry['balance'],
                'curr_balance': values['curr_balance'] + entry['curr_balance'],
            })

    def _compute_init_balance(self, account_id=None, period_ids=None,
                              mode='computed', default_values=False):
        """ mode and default_values parameters are deprecated AFAICS """
